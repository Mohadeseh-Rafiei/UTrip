...
class Tool:
    @staticmethod
    def calculate_average(sum_of_items: float, num_of_items: float) -> float:
        return sum_of_items / num_of_items


class Rating:
    def __init__(self) -> None:
        self.number_of_rating = 0
        self.location = 0
        self.cleanliness = 0
        self.staff = 0
        self.facilities = 0
        self.value_for_money = 0
        self.overall_rating = 0

    def add_rating(self, location: float, cleanliness: float, staff: float, facilities: float, value_for_money: float,
...
class Comment:
    def __init__(self, username: str, comment: str) -> None:
...
class HotelsManager:
    ...
    def add_comment_to_hotel(self, username: str, hotel_id: str, comment: str) -> None:
        self.all_hotel[hotel_id].comments.append(Comment(username, comment))

    def post_rating_to_hotel(self, hotel_id: str, location: float, cleanliness: float, staff: float, facilities: float,
                             value_for_money: float, overall_rating: float) -> None:
        self.all_hotel[hotel_id].rating.add_rating(location, cleanliness, staff, facilities, value_for_money,
                                                   overall_rating)

    def get_comment_hotel(self, hotel_id: str) -> list:
        return self.all_hotel[hotel_id].comments

    def get_ratings_hotel(self, hotel_id: str) -> Rating:
        return self.all_hotel[hotel_id].rating
...
class UtRip:
    ...
    def post_comment(self, hotel_id: str, comment: str) -> None:
        if self._user_manager.current_user is None:
            raise Exception("permission_denied")
        self._hotels_manager.add_comment_to_hotel(self._user_manager.current_user.username, hotel_id, comment)

    def post_rating(self, hotel_id: str, location: float, cleanliness: float, staff: float, facilities: float,
                    value_for_money: float, overall_rating: float) -> None:
        if self._user_manager.current_user is None:
            raise Exception("permission_denied")
        self._hotels_manager.post_rating_to_hotel(hotel_id, location, cleanliness, staff, facilities, value_for_money,
                                                  overall_rating)

    def get_comment(self, hotel_id: str) -> list:
        if self._user_manager.current_user is None:
            raise Exception("permission_denied")
        return self._hotels_manager.get_comment_hotel(hotel_id)

    def get_ratings(self, hotel_id: str) -> Rating:
        return self._hotels_manager.get_ratings_hotel(hotel_id)
...
class UserInterface:
            ...
            "POST":
                {
                    ...
                    "comments": self._post_comment,
                    "ratings": self._post_rating,
                },
            "GET":
                {
    ...
    @staticmethod
    def _print_average_rating(rating: Rating) -> None:
        print("location:", Tool.calculate_average(rating.location, rating.number_of_rating))
        print("cleanliness:", Tool.calculate_average(rating.cleanliness, rating.number_of_rating))
        print("staff:", Tool.calculate_average(rating.staff, rating.number_of_rating))
        print("facilities:", Tool.calculate_average(rating.facilities, rating.number_of_rating))
        print("value for money:", Tool.calculate_average(rating.overall_rating, rating.number_of_rating))

    def _get_ratings(self, data: list) -> None:
        hotel_id = data[data.index("hotel") + 1]
        try:
            rating = self._ut_rip.get_ratings(hotel_id)
            self._print_average_rating(rating)
    ...
    def _get_comments(self, data: list) -> None:
        hotel_id = data[data.index("hotel") + 1]
        try:
            comments = self._ut_rip.get_comment(hotel_id)
            self._print_comments(comments)
        except Exception as err:
            print(self._messages[str(err.args[0])])

    def _post_rating(self, data: list) -> None:
        hotel_id = data[data.index("hotel") + 1]
        location = float(data[data.index("location") + 1])
        cleanliness = float(data[data.index("cleanliness") + 1])
        staff = float(data[data.index("staff") + 1])
        facilities = float(data[data.index("facilities") + 1])
        value_for_money = float(data[data.index("value_for_money") + 1])
        overall_rating = float(data[data.index("overall_rating") + 1])
        try:
            self._ut_rip.post_rating(hotel_id, location, cleanliness, staff, facilities, value_for_money,
                                     overall_rating)
            print("OK")
        except Exception as err:
            print(self._messages[str(err.args[0])])

    def _post_comment(self, data: list) -> None:
        hotel_id = data[data.index("hotel") + 1]
        comment = data[data.index("comment") + 1]
        try:
            self._ut_rip.post_comment(hotel_id, comment)
            print("OK")
        except Exception as err:
            print(self._messages[str(err.args)])
    ...
    def run(self) -> None:
        self._read_hotels_data("Assets/Hotels.csv")
        self._get_command()
